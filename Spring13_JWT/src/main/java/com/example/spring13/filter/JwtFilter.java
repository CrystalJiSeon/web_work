...
@Component
public class JwtFilter extends OncePerRequestFilter {//OncePerRequestFilter 클래스를 상속 받는다
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		//요청의 Header에 "Authorization"이라는 키값으로 전달된 문자열이 있는지 읽어와 본다.
		String authHeader=request.getHeader("Authorization");
		if(authHeader !=null &&authHeader.startsWith("Bearer ")) {
			jwtToken= authHeader.substring(7);
		}
		if(jwtToken !=null) {//토큰이 존재하고
			if(util.validateToken(jwtToken)) {//유효성 검증을 통과한다면
				//Spring Security에 1회성 로그인 처리를 해주고 요청의 흐름을 이어간다
