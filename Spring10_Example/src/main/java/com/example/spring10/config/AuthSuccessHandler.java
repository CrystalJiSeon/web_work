...
public class AuthSuccessHandler extends SavedRequestAwareAuthenticationSuccessHandler{
	...
	public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response,
        ...
        session.setMaxInactiveInterval(60*20);//초단위로 설정->20분 동안 세션이 유지
        
        //Authentication 객체의 메소드를 이용해서 지금 로그인된 사용자에 대한 자세한 정보를 얻어낼수 있다.
        String userName=authentication.getName();
        System.out.println("로그인된 사용자:"+userName);
		
        //로그인된 userName을 세션 영역에 담아놓으면 jsp 또는 thymeleaf에서 편리하게 사용할 수 있다.
        //jsp에서는 ${sessionScope.userName} 또는 ${userName}으로 참조
        //타임리프에서는 ${session.userName}으로 userName을 추출해서 참조할 수 있음
        ...
        if(cashed==null) {
        	//5. 로그인 환영 페이지로 foward 이동 (POST 방식 요청임에 주의!!!) Controller 필요
