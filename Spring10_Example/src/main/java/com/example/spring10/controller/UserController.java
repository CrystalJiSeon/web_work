...
import com.example.spring10.service.UserSerivce;
...
public class UserController {
	...
	@Autowired private UserSerivce service;
	...
	public Map<String, Boolean> checkid(String userName){
		//get 방식 파라미터로 전달되는 userName을 이용해서 UserDto를 얻어와본다.
		UserDto dto=service.getByUserName(userName);
		//dto가 null이 면 사용 가능, 사용 불가능
		boolean canUse=dto ==null? true: false;
		//Map 객체에 사용 가능 여부를 담아서 리턴한다.
		Map<String, Boolean> map = Map.of("canUse", canUse);
		return map;
		//return Map.or("canUse", canUse);라고 두줄을 한줄로 작성해도 됨.
	}
